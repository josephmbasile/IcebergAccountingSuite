...
class new_session:
    def __init__(self):
        ...
        self.ledger_tables = [f"{current_year}"]
...
layout1 = [
    ...
    [sg.Column([[sg.Text("Year: ", font=("",medium_print)),sg.OptionMenu(values=["CY2024"], enable_events=True, auto_size_text=True, default_value="CY2024",key="-Year_Picker-")]], justification="center", element_justification="center")],
...
def create_database(values, current_console_messages,window, num, current_year):
   ...
   #7 Create the Console_Log Table
    ...
    icb_session.ledger_tables = [f"{current_year}"]
    ...
    database_properties = [
        ...
        ["Ledgers",icb_session.ledger_tables,"",icb_session.current_time_display[0],icb_session.current_time_display[0]],
    ]
...
def update_dashboard_statistics(window, values):
    current_year = values['-Year_Picker-']
    ...
    #latest_year = window["-Year_Picker-"].values[-1]
...
def load_view_account_tab(window, values, account_number, ledger_name):
    ...
    this_account_transactions_query = f"""SELECT * FROM {ledger_name} WHERE Debit_Acct IS {account_number} OR Credit_Acct IS {account_number} ORDER BY Created_Time DESC;"""
    ...
    if event == '__TIMEOUT__':
        ...
        #    current_console_messages = console_log(icb_session.window, f"""Filekey: {filekey}, filename: {filename}""",connection,current_console_messages)
            ...
            if event_opendb == f"-Open_Database_Button_{icb_session.num}-": 
                ...
                icb_session.ledger_name = f"tbl_ledger_{icb_session.db_name[:-4]}_CY{current_year}"
                #print(icb_session.db_name)
