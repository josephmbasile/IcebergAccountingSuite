...
You can update the status of your invoice from the Customers>Point of Sale tab.

   > Select your invoice from the list.

   > Select the Edit Invoice button to unlock the status field.

   > Update the status. 
   
   > Select the Save Invoice button. 
   
If you select Paid on a previously Due or Overdue invoice, the system will ask for a payment account where you received the money and add transactions as appropriate. Similarly, if you refund a previously Paid invoice, it will ask for a payment account that acted as the source of the refund and add those transactions. Finally, if you Cancel a Due or Overdue Invoice, it will add transactions to cancel out the original transactions generated by the invoice.
